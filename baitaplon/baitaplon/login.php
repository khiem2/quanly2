...
if(isset($_GET['status'])&&$_GET['status']=="false")
    echo "<script>alert('Bạn đã đăng nhập sai ,mời bạn đăng nhập lại')</script>";
 ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="login-form">
        <form class="loginTab signin" <?php 
        if(isset($_GET['role'])&&$_GET['role']==1){echo'action="xulylogin_quantri.php"';}
        if(isset($_GET['role'])&&$_GET['role']==2){echo'action="xulylogin_quanly.php"';}
        if(isset($_GET['role'])&&$_GET['role']==3){echo'action="xulylogin_giangvien.php"';}
        if(!isset($_GET['role'])){echo'action="login.php"';}
         ?> method="POST">
